...
public class Say {
    ...
    private void getHundredsName(int number, StringBuilder sb) {
        ...
        String[] units = String.valueOf(number).split("");
        int firstUnit = Integer.parseInt(units[0]);
        int remainingUnits = Integer.parseInt(units[1] + units[2]);

        getUnitName(firstUnit, sb);
        sb.append(" hundred");

        if(remainingUnits != 00) {
            sb.append(" ");
    ...
    private void getMillionName(int number, StringBuilder sb) {
        ...
        String stringNumber = String.valueOf(number);
            int firstUnit = Integer.parseInt(stringNumber.substring(0, 1));
            String firstHundred = stringNumber.substring(1, 4);

            getUnitName(firstUnit, sb);
            sb.append(" million");

            if(!firstHundred.equals("000")) {

                char firstNumber = firstHundred.charAt(0);
                char secondNumber = firstHundred.charAt(1);

                if(firstNumber == '0') {
                    
                    sb.append(" ");
                    getThousandsName(Integer.parseInt(stringNumber.substring(2, stringNumber.length())), sb);
                    
                } else if(firstNumber == '0' && secondNumber == '0') {
                    
                    sb.append(" ");
                    getThousandsName(Integer.parseInt(stringNumber.substring(5, stringNumber.length())), sb);
        ...
        if(stringNumber.length() == 10) {
            ...
            for(int i = 0; i < stringNumber.length(); i += 3) {
                ...
                if(i == 0) {
                    ...
                    hundred = stringNumber.substring(0, 3);
                    getHundredsName(Integer.parseInt(hundred), sb);
                    sb.append(" billion ");
                    
                } else {

                    hundred = stringNumber.substring(i, i + 3);
                    getHundredsName(Integer.parseInt(hundred), sb);
